...
def heqie(hand, output_notes=False):
    ...
    xiangtingshu_14, num_youxiaopai, list_youxiaopai = mahjong_checker.cal_xiangtingshu(hand, raw_hand=False)
    ...
    for card in hand: # 循环打每一张牌, 判断哪张向听数最小且有效牌多.
        ...
        xiangtingshu, num_youxiaopai, list_youxiaopai = mahjong_checker.cal_xiangtingshu(hand_card, raw_hand=False)
    ...
    if output_notes: #输出调试信息
        mahjong_checker.print_hand(hand) # 输出手牌内容
        youxiaopai = ''
...
def heqie_old(hand):
    ...
    o: 打某张 or 宣布和牌
    """
    if mahjong_checker.mahjong_checker(hand, output_notes=False, raw_hand=False):
        # 已经和牌
        print("和牌")
        return False
    else:
...
def heqie_tester():
    ...
    while paishan:
        ...
        mahjong_checker.used_card(new_card) # 计算剩余牌量
        print 'hand: '
        hand.sort(key=mahjong_checker.sort_hand)
        mahjong_checker.print_hand(hand)
        print('new card', new_card)
        hand = hand + [mahjong_checker.Card(new_card)]
        ...
        if discard_card:
            for card in hand:
                if mahjong_checker.is_samecard(card, discard_card):
                    hand.remove(card)
                    break
        else:
            print('和牌')
...
def main():
    ...
    script, hand = argv
    heqie(mahjong_checker.checker.hand_processer(hand),output_notes=True)
