...
#
#         print mahjong_checker.mahjong_checker(case)
...
def istingpai(hand, raw_hand=False, output_notes=False):
    ...
    """
    if raw_hand:
        hand = mahjong_checker.hand_processer(hand, length=13, check_input=True)
    if hand:
        flag = False
        cardlist_of_tingpai = []
        for card in CARD_LIST:
            hand_card = hand + [mahjong_checker.Card(card)]
            hand_card = mahjong_checker.hand_processer(hand_card, raw_hand=False) #sort again
            if mahjong_checker.mahjong_checker(hand_card, output_notes=False, raw_hand=False):
                if output_notes:
                    print card
...
def totingpai_14(hand, raw_hand=False, output_notes=False):
    """判断14张牌打掉哪张/哪些可以听牌
    """
    hand = mahjong_checker.hand_processer(hand, raw_hand=raw_hand, length=14, check_input=True)
...
def isyixiangting(hand, raw_hand=True):
    """判断13张牌是否是一向听
    """
    hand = mahjong_checker.hand_processer(hand, raw_hand=raw_hand, length=13, check_input=True)
    ...
    if istingpai(hand, raw_hand=False, output_notes=False): #已经听牌, 直接返回
        ...
        for card in CARD_LIST:
            hand_card = hand[:]
            hand_card.append(mahjong_checker.Card(card))
...
if __name__ == '__main__':
    # print test_cases()
    # print istingpai([mahjong_checker.Card('112345678999m5p')])

    #istingpai(mahjong_checker.hand_processer('1112345876999m'),raw_hand=False)

    # print str(totingpai_14([mahjong_checker.Card('123456789m1233p')]))
    ...
    isyixiangting('12344688m123p22s')
