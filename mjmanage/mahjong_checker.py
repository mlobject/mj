...
class Card :
    ...
    def __init__(self, card) :
        # super(Card, self).__init__() python 3.*支持
...
MIANZI = ['shunzi', 'kezi']
QUETOU = ['duizi']
DAZI = ['duizi', 'bianzhang', 'kanzhang', 'liangmian']  # 搭子
...
CARD_LIST_T = [str(rank) + suit for suit in ['m', 'p', 's'] for rank in range(1, 10)]

CARD_LEFT_T = {card : 3 for card in CARD_LIST}


def init_paishan() :
    # 生成140张牌山
    # 有序数牌(含万子牌、饼子牌和条子牌)108张、风牌(即东、南、西、北)16张，箭牌 (中、发、白)12张 , 春夏秋冬
    # i: nothing
    # o: a list of 136 random card
    paishan_list = CARD_LIST * 4 + ["1t", "2t", "3t", "4t"]
...
def init_paishan_T() :
    # 生成112张牌山
    # 有序数牌(含万子牌、饼子牌和条子牌)108张 春夏秋冬
    # i: nothing
    # o: a list of 136 random card
    paishan_list = CARD_LIST_T * 4 + ["1t", "2t", "3t", "4t"]
    random.shuffle(paishan_list)
    return paishan_list


def used_card(card) :
    # 从字典中去掉用过的牌, card in Card class
    CARD_LEFT[str(card)] -= 1


VALID_LENGTH_OF_HAND = 14  # 由库中记录保障
MIANZI_MAX = 4  # 一副牌中最多4个面子
QUETOU_MAX = 1  # 胡牌只能有一个雀头 将
XIANGTINGSHU_MAX = 8  # ?
...
class Mianzi(object) :
    ...
    def isvalid(self) :
        if (self.card1.suit == self.card2.suit and self.card1.suit == self.card3.suit) :
            if (self.card1.rank == self.card2.rank - 1 and self.card2.rank == self.card3.rank - 1 and self.card1.suit is not 'z') :  # 顺子不能是字牌
                return True
...
def hand_processer(hand, raw_hand=True, check_input=False) :
...
def mahjong_checker(hand, output_notes=False, raw_hand=True) :
    ...
    # print "hand : " + str(hand)
    # print "hand : " + (Card(f).get_rank() + Card(f).get_suit() for f in hand)
    if hand :
        # 1. non standard form
...
# print init_paishan_T()
# print init_paishan()
#
# print len(init_paishan_T())
# print len(init_paishan())

# print mahjong_checker('12333456m111222z', output_notes=True)
